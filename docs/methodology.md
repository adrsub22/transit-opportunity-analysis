...
3. **Harmonization uses area weighting.** Where population is unevenly distributed within a redrawn 2010 unit, area weighting is a coarse approximation. Population-weighted apportionment (using 2010 decennial block populations) is more accurate.
4. **LODES suppression.** LODES uses differential-privacy noise injection. Small block groups can have noisy or undercounted jobs.
5. **Disability is tract-level.** See above.
6. **Index weights are not empirically validated.** They're defensible, not derived. A natural next step is to regress index components against observed ridership where the agency has APC/AFC data.

---

## What I'd add next

- **Population-weighted harmonization.** Replace area weighting with weights derived from 2010 decennial block populations — more accurate where density varies sharply within a redrawn unit.
